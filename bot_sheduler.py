...
async def send_request_message(
    ...
    try:
        if last:
            text = """Понравились картинки? Закажи ещё 😈
Также ты можешь поменять свое фото и пол в /settings и попробовать снова 👇
            """
            async with limiter:
                await limiter.acquire(2)
                await bot.send_message(chat_id=chat_id, text=text, reply_markup=keyboard_select_other_tarif)
    except (UserDeactivated, ChatNotFound, BotBlocked) as e:
        print(f"Пользователь c id: {chat_id} заблокировал бота")
    except RetryAfter as e:
...
async def check_for_ready_queue(dp: Dispatcher):
    ...
    if (
        ...
        for i, target_image_link in enumerate(target_image_links):
            print('enumerate', enumerate(target_image_links), 'target', target_image_links)
