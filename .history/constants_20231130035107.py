...
class SingleMessageMiddleware(BaseMiddleware):
    async def on_pre_process_message(self, message: types.Message, data: dict):
        ...
        chat = await message.bot.get_chat(chat_id=message.chat.id)

        # Получаем список сообщений в чате
        messages = await message.bot.get_messages(chat_id=message.chat.id, limit=10)

        # Удаляем все предыдущие сообщения, кроме последнего
        for old_message in messages:
            if old_message.message_id != messages[-1].message_id:
                await message.bot.delete_message(chat_id=message.chat.id, message_id=old_message.message_id)
...
dp.middleware.setup(SingleMessageMiddleware())
