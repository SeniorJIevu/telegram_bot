...
async def button_click_handler(query: CallbackQuery):
    button_id = query.inline_message_id
    current_time = time.time()

    if button_id in last_click_time and current_time - last_click_time[button_id] < 10:
        await bot.answer_callback_query(query.id, text="Вы уже нажали кнопку", show_alert=True)
    else:
        last_click_time[button_id] = current_time
...
class ButtonMiddleware(BaseMiddleware):
    async def on_process_callback_query(self, query: CallbackQuery, data: dict):
        await button_click_handler(query)
