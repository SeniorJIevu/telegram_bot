...
### Check button click
async def check_time_click(message, time, button_message,  db: asyncpg.pool.Pool):
    async with db.acquire() as connection:
            async with connection.transaction():
                message_btn = await connection.fetchval(
                    f"SELECT {button_message} FROM users WHERE user_id = $1",
                    message.chat.id,
                )
                
                print(message_btn, type(message_btn))
                times = time - message_btn  
                print(times.seconds)
                
                if message_btn is None:
                    return True
                else:
                    if (time - message_btn).seconds >= 10:
                        return True 
                    else:
                        return False
...
async def check_if_use_free_tarif(message, db: asyncpg.pool.Pool):
    """
    ...
    query = """
        ...
        WHERE user_id=$1 AND image_tarif=$2
    """
    if message == types.CallbackQuery:
        async with db.acquire() as connection:
            result = await connection.fetch(query, message.message.chat.id, IMAGE_FREE)
    else:
        async with db.acquire() as connection:
            result = await connection.fetch(query, message.message.chat.id, IMAGE_FREE)
    return len(result) > 0
