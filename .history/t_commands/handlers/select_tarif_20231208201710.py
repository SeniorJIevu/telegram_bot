    ...
    if history_image_links != {}:
        for link in history_image_links:
            try:
                ...
                continue
    print('copy_deepcopy', user_references)
...
Пожалуйста пройдите авторизацию, нажав команду /start
    ...
    print('get_user_references_remaining', user_references)
    print('get_user_references_remaining', user_references, current_user_image_link)
...
@dp.callback_query_handler(lambda el: el.data == "🍓 Сочное за: 10$ 🍓", state=TarifStart.send_select_tarif)
...
Пожалуйста пройдите авторизацию, нажав команду /start
    ...
    price = types.LabeledPrice(label="10 фоточек", amount=TURBO_RUB)
    photo_url = (
...
На коллаже снизу показаны стили твоих будущих аватарок
    ...
    print('ДЛИНА', len(user_references))
    if len(user_references):
        await bot.send_invoice(
            ...
            payload="tarif-turbo",
            provider_data='{"receipt": {"items": [{"description": "10 фоточек", "quantity": "1", "amount": {"value": "200.00", "currency": "RUB"}, "vat_code": 1}], "customer": {"email": "best.web.partners@gmail.com"}}}',
        )
