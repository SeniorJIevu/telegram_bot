...
async def сonfirmation_age(
    ...
    try:
        if not await get_user_information(message.chat.id, db):
            await message.answer(text, reply_markup=keyboard_conf_age)
            await state.set_state(OrderStart.send_start.state)
        else:
                        # Переключение на состояние send_start
            async with state.proxy() as data:
                data['some_key'] = 'some_value'  # Вы можете сохранить любые необходимые данные в состоянии
            await OrderStart.send_start.set()
            await send_start(types.CallbackQuery, state, db)
...
def register_handler_start(dp: Dispatcher):
    dp.register_message_handler(сonfirmation_age, commands="start", state="*")
