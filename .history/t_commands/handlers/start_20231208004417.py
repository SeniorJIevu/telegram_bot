...
async def start(message: types.Message, state: FSMContext,  db: asyncpg.pool.Pool):
    ...
    async with aiofiles.open(
                        "images/confirm_age.png", mode="rb") as photo:
        ...
        await state.update_data(message_id=send_message.message_id, chat_id=send_message.chat.id)
...
async def handle_start(message: types.Message, state: FSMContext, db: asyncpg.pool.Pool):
    ...
    await update_start_message(state)
...
@rate_limit(5)
async def send_start(message: types.Message, state: FSMContext, db: asyncpg.pool.Pool):
    message_data = await state.get_data()
    print(type(message_data['message_id']))
...
async def update_start_message(state: FSMContext):
    message_data = await state.get_data()
...
async def send_continue(
    ...
    try:
        ...
        if message.text.lower() != "продолжить 👉🏻":
            ...
            return
        await message.answer(text, reply_markup=keyboard_select_gender)
        await state.set_state(OrderStart.send_continue.state)
    except (UserDeactivated, ChatNotFound, BotBlocked) as e:
        print(f"Пользователь c id: {message.chat.id} заблокировал бота")
...
async def send_gender(
    ...
    if image_count >= 1:
        ...
        return
    if message.text.lower() not in ("🚺 женский", "🚹 мужской"):
        await message.answer(
            (
        ...
        return
    if message.text.lower() == "🚺 женский":
        await user_update_gender(message.chat.id, "female", db)
    else:
        await user_update_gender(message.chat.id, "male", db)
...
async def click_continue(
    ...
    if (
        message.text.lower() != "продолжить 👉🏻"
    ) and message.text.lower() not in ("🚺 женский", "🚹 мужской"):
        await message.answer(
            "Чтобы продолжить нажмите кнопку ниже 👇🏼",
...
def register_handler_start(dp: Dispatcher):
    ...
    dp.register_message_handler(send_start,state=OrderStart.send_start)
    dp.register_message_handler(send_continue, state=OrderStart.send_start)
    dp.register_message_handler(send_gender, state=OrderStart.send_continue)
