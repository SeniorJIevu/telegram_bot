        ...
        if is_user_exist:
            ...
            async with aiofiles.open(
                ...
                async with db.acquire() as connection:
                    connection.execute(
                               "UPDATE users SET msg_id = $1 WHERE user_id = $2",
                               msg_id, 
                               message.chat.id
                               )  
                    connection.close() 
                    
            await state.set_data('msg_start', msg_id)
...
async def click_continue(
    ...
    try:
        ...
        async with db.acquire() as connection:
            msg_id = await connection.fetchval(
                    "SELECT msg_id FROM users WHERE user_id = $1",
                    call.message.chat.id,
                )
        connection.close()   
    except Exception as error_tarif:
        print('error_tarif',error_tarif)   
    await bot.delete_message(chat_id=call.message.chat.id, message_id = msg_id)
    a = state.get_state('msg_start')
    print('ПриветЖЖ')
    print('msg_id_try',msg_id)
    
    text='Тестовый текст'
    
    async with aiofiles.open(
        "images/photo_start.jpg", mode="rb") as photo:
            msg_id = await call.message.edit_media(
                    photo=photo,
                    caption=text,
                    reply_markup=keyboard_send_start,
                )
    ...
    async with aiofiles.open(photo_tarif, mode="rb") as photo:
        msg_tarif = await call.message.answer_photo(
            photo=photo,
            caption="Выбери тариф ☝️",
            reply_markup=keyboard,
        )
        msg_id = msg_tarif.message_id
        print('msg_id', msg_id) 
        connect_to_db()
        async with db.acquire() as connection:
            connection.execute(
