    ...
    async def wrapper(message_or_callback_query, state, db):
        ...
        if user_id in button_clicks:
            ...
            if button_info['timestamp'] + cooldown_time > time.time():
                ...
                return
            elif button_info['message_id']:
                # Для этого пользователя уже было отправлено сообщение, проверяем, прошло ли достаточно времени
                sent_message = await message_or_callback_query.bot.send_message(user_id, "You clicked the button! Do something here...")
                if sent_message.message_id == button_info['message_id']:
                    # Сообщение было отправлено в течение cooldown_time секунд, подождите
