...
async def start(message: types.Message, state: FSMContext,  db: asyncpg.pool.Pool):
    ...
    if await check_time_click(message, time_now, 'start_message', db):
        print('Работает')
        ...
        text= """
        Не спамьте, ждите 10 секунд..."""
        #await message.answer(text)
        await bot.answer_pre_checkout_query(message.from_user.id, text)        

@dp.callback_query_handler(lambda call: call.data == "Начать", state=OrderStart.send_start)
async def handle_start(call: types.CallbackQuery, state: FSMContext, db: asyncpg.pool.Pool):
    time_now = datetime.now()
    if await check_time_click(call, time_now, 'start_message', db):
        text="""
            Возраст подтверждён!
        """
        await call.message.answer(text)
        await send_start(call, state, db)
