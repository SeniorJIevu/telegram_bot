...
def restrict_button_click_interval(func):
    @wraps(func)
    async def wrapper(*args, **kwargs):
        user_id = None
        query = None

        if "callback_query" in kwargs and isinstance(kwargs["callback_query"], types.CallbackQuery):
            user_id = kwargs["callback_query"].from_user.id
            query = kwargs["callback_query"]
        elif "message" in kwargs and isinstance(kwargs["message"], types.Message):
        ...
        # Проверяем, прошло ли достаточно времени с момента последнего нажатия кнопки
        current_time = time.time()
        last_click_time = last_click_times.get(user_id, 0)

        cooldown_time = 10  # Время ожидания между нажатиями в секундах

        if current_time - last_click_time < cooldown_time:
            await query.answer("Пожалуйста, подождите перед следующим нажатием кнопки.")
            return
