    ...
    async def wrapper(message_or_callback_query, state, db):
        ...
        if user_id in button_clicks:
            ...
            if button_info['timestamp'] + cooldown_time > time.time():
                ...
                return
            elif button_info['message_sent']:
                # Для этого пользователя уже было отправлено сообщение
        ...
        button_clicks[user_id] = {
            'timestamp': time.time(),
            'message_sent': True,
            'button': getattr(message_or_callback_query, 'data', None),  # При callback_query это data, при message - None
        }
