...
@dp.callback_query_handler(lambda el: el.data == "То что вам нужно 10: 10$", state=TarifStart.send_select_tarif)
...
Пожалуйста пройдите авторизацию, нажав команду /start
    ...
    text = f"""Тариф Супер
Получить 10 аватарок в течение нескольких часов

В отличие от бесплатного тарифа ты получишь аватарки в 10 разных стилях.
На коллаже снизу показаны стили твоих будущих аватарок
    ...
    if len(user_references) >= COUNT_PHOTO_TURBO:
        await bot.send_invoice(
            ...
            payload="tarif-turbo",
            provider_data='{"receipt": {"items": [{"description": "10 аватарок", "quantity": "1", "amount": {"value": "200.00", "currency": "RUB"}, "vat_code": 1}], "customer": {"email": "best.web.partners@gmail.com"}}}',
        )
