...
from t_commands.handlers.start import OrderStart, send_continue
...
@dp.callback_query_handler(lambda el: el.data == 'Настройки', state=TarifStart.send_select_tarif)
async def settings_call(
    call: types.CallbackQuery, state: FSMContext, db: asyncpg.pool.Pool
):
    print(call.data, 'Настройки')
...
def register_settings(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        ...
        statee=TarifStart.send_select_tarif,
    )
    dp.register_message_handler(
        back_to_tarif,
