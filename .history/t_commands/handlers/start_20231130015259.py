...
# Define cooldown time for preventing double clicks
cooldown_time = 10  # seconds

# Dictionary to store last sent messages in each chat
last_sent_message = {}

# Decorator to prevent double click and implement anti-flood protection
def prevent_double_click(func):
    async def wrapper(message: types.Message, state: FSMContext):
        user_id = message.from_user.id
        chat_id = message.chat.id

        # Prevent repeated messages in the chat
        if chat_id in last_sent_message and last_sent_message[chat_id] == message.text:
            return

        # Check if the user is sending messages too quickly (anti-flood protection)
        try:
            await func(message, state)
        except Throttled as e:
            # Handle anti-flood throttling
            await message.reply("You are sending messages too quickly. Please wait a moment.")
            return

        # Update the last sent message for the chat
        last_sent_message[chat_id] = message.text
