...
from constants import back_limit, rate_limit
...
if str(CallbackQuery.from_user.id) in list_user_id:
    CallbackQuery.message.answer('Пожалуйста подождите...')
else:     
    @dp.callback_query_handler(state=OrderStart.send_continue)
    async def click_continue(
        call: types.CallbackQuery, state: FSMContext, db: asyncpg.pool.Pool
    ):  
            list_user_id[str(call.from_user.id)] == call.message.chat.id 
            if (
