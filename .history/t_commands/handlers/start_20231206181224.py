...
@dp.message_handler(state=OrderStart.confirmed_age)
async def send_start(
    ...
    try:
        print(message.text)
        if not await get_user_information(message.chat.id, db) t:
            await message.answer(text1, reply_markup=keyboard_conf_age)
            await state.set_state(OrderStart.send_age.state)

        elif not await get_user_information(message.chat.id, db) and message.text == "Мне есть 18 лет!":
            print('b')
            text = """Привет ✌
...
async def send_age(message: types.Message, state: FSMContext, db: asyncpg.pool.Pool):
    text1 = "Вам уже исполнилось 18 лет ? Данный бот не для детей :)"
    keyboard_conf_age = ReplyKeyboardMarkup(resize_keyboard=True, row_width=1) 
    button_2 = KeyboardButton(text=TRUE_AGE)
    button_4 = KeyboardButton(text=FALSE_AGE)
    keyboard_conf_age.add(button_2, button_4)
    
    try:
        if message.text == "Мне есть 18 лет!":
            print('a')
...
def register_handler_start(dp: Dispatcher):
    ...
    dp.register_message_handler(send_start, state=OrderStart.confirmed_age)
