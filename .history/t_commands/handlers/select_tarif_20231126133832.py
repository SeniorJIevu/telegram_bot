...
from t_commands.handlers.settings import OrderSettings,settings_call
...
async def add_to_queue(call, request_type, count_photo, db):
    ...
    id_in_user_images = await add_pending_image_request(
        call.from_user.id, request_type, db
    )
...
@dp.callback_query_handler(lambda el: el.data == 'Бесплатно')
...
Пожалуйста пройдите авторизацию, нажав команду /start
    ...
    if await check_if_use_free_tarif(call, db):
        ...
        return
    await add_to_queue(call, IMAGE_FREE, COUNT_PHOTO_FREE, db)
...
async def send_select_tarif(
    ...
    if call.data == 'Настройки':
        await settings_call(call, state, db)
    else:    
        text = "Выбери тариф"
