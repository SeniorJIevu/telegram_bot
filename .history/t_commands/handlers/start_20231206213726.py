...
async def start(message: types.Message, state: FSMContext):
    await OrderStart.waiting_for_start.set()    
...
@dp.message_handler(state=OrderStart.waiting_for_start, commands=['settings'], text='/settings')
async def process_settings_command(message: types.Message):
    await message.reply("Команда /settings недоступна в данный момент.")
