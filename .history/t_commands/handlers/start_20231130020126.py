...
def prevent_multiple_executions(func):
    @wraps(func)
    async def wrapper(*args, **kwargs):
        user_id = None

        # Определяем идентификатор пользователя в зависимости от типа обработчика
        ...
        last_execution_time = last_execution_times.get(user_id, 0)

        cooldown_time = 10  # Время ожидания после отправки сообщения в секундах
        if current_time - last_execution_time < cooldown_time:
            if "message" in kwargs and isinstance(kwargs["message"], types.Message):
                await kwargs["message"].answer("Подождите немного перед повторным запросом.")
            elif "callback_query" in kwargs and isinstance(kwargs["callback_query"], types.CallbackQuery):
                await kwargs["callback_query"].answer("Подождите немного перед повторным запросом.")
            return
