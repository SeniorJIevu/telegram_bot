...
async def delete_previous_message(chat_id, message_id, bot):
    try:
        await bot.delete_message(chat_id=chat_id, message_id=message_id)
    except Exception as e:
        print(f"Failed to delete previous message: {e}")

def delete_previous_message_decorator(func):
    @wraps(func)
    async def wrapper(*args, **kwargs):
        ...
        if user_id in sent_messages and message_chat_id in sent_messages[user_id]:
            await delete_previous_message(message_chat_id, sent_messages[user_id][message_chat_id], args[0].bot)
