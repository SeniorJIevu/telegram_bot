...
async def send_continue(
    ...
    data_ms_photo_id = data.get('message_photo_id')
    if data_ms_photo_id:
        await bot.delete_message(chat_id=call.message.chat.id, message_id=data_ms_photo_id)
    try:
...
async def send_gender(
    ...
    await bot.delete_message(chat_id=call.message.chat.id, message_id=data_ms_photo)
