...
Очередь — {queue} человек.
    ...
    await add_to_queue(call.message, IMAGE_COLLAGE_50, COUNT_PHOTO_TURBO, db)
...
@dp.callback_query_handler(lambda el: el.data == "get_18+_avatarok", state=TarifStart.send_select_tarif)
...
Пожалуйста пройдите авторизацию, нажав команду /start
    ...
    if len(user_references) >= COUNT_PHOTO_PREM:
        await bot.send_invoice(
            ...
            provider_data='{"receipt": {"items": [{"description": "100 аватарок", "quantity": "1", "amount": {"value": "400.00", "currency": "RUB"}, "vat_code": 1}], "customer": {"email": "best.web.partners@gmail.com"}}}',
        )
        if str(call.message.chat.id) in admin_id:
            await call.message.answer(
                "Только для админа",
