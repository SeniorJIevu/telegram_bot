...
def prevent_double_click(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        # Определяем идентификатор пользователя и chat_id сообщения в зависимости от типа обработчика
        if "message" in kwargs and isinstance(kwargs["message"], types.Message):
            user_id = kwargs["message"].from_user.id
            message_chat_id = kwargs["message"].chat.id
        elif "callback_query" in kwargs and isinstance(kwargs["callback_query"], types.CallbackQuery):
            user_id = kwargs["callback_query"].from_user.id
            message_chat_id = kwargs["callback_query"].message.chat.id

        if user_id is None or message_chat_id is None:
            return

        # Проверяем, было ли отправлено предыдущее сообщение, и удаляем его
        if user_id in sent_messages and message_chat_id in sent_messages[user_id]:
            previous_message_id = sent_messages[user_id][message_chat_id]
            try:
                await args[0].bot.delete_message(chat_id=message_chat_id, message_id=previous_message_id)
        ...
        # Обновляем информацию о последнем отправленном сообщении
        if result and "message_id" in result:
            if user_id not in sent_messages:
                sent_messages[user_id] = {}
            sent_messages[user_id][message_chat_id] = result["message_id"]
