...
# Словарь для отслеживания, была ли кнопка уже нажата для каждого пользователя
button_clicks = {}

# Время, которое пользователь должен подождать перед повторным нажатием кнопки (в секундах)
cooldown_time = 60
...
def prevent_double_click(func):
    async def wrapper(message_or_callback_query, state: FSMContext, db: asyncpg.pool.Pool):
        user_id = message_or_callback_query.from_user.id

        # Проверяем, была ли кнопка уже нажата и было ли уже отправлено сообщение
        if user_id in button_clicks and (time.time() - button_clicks[user_id]['timestamp']) < cooldown_time:
            await message_or_callback_query.answer("Вы уже нажали кнопку, пожалуйста, подождите :)")
        elif user_id in button_clicks and button_clicks[user_id]['message_sent']:
            await message_or_callback_query.answer("Вы уже отправили сообщение, подождите ответа :)")
        else:
            # Если кнопка не была нажата или прошло достаточно времени,
            # вызываем функцию-обработчик и обновляем информацию о нажатии
            await func(message_or_callback_query, state, db)
