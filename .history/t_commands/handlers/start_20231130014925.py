...
max_messages_per_cooldown = 1  # Максимальное количество отправленных сообщений в течение cooldown_time
...
def prevent_double_click(func):
    @wraps(func)
    async def wrapper(message_or_callback_query, state, db):
        ...
        if user_id in button_clicks:
            ...
            if button_info['timestamp'] + cooldown_time > time.time():
                ...
                await message_or_callback_query.answer("Вы слишком быстро нажали на кнопку, подождите :)")
                return
                ...
                return
            elif button_info['button_clicked']:
                # Пользователь уже успешно нажал на кнопку, игнорируем повторные нажатия
                return
        ...
        button_clicks[user_id] = {
            ...
            'button_clicked': True,  # Флаг успешного нажатия кнопки
        }

        # Добавляем идентификатор отправленного сообщения в словарь отправленных сообщений
        sent_message_id = message_or_callback_query.message.message_id if hasattr(message_or_callback_query, 'message') else None
        if sent_message_id:
