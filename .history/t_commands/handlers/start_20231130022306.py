...
async def delete_previous_messages(chat_id, bot):
    try:
        # Получаем список сообщений в чате
        messages = await bot.get_chat_history(chat_id, limit=10)

        # Удаляем все предыдущие сообщения, кроме последнего
...
def delete_previous_messages_decorator(func):
    @wraps(func)
    async def wrapper(*args, **kwargs):
        ...
        # Удаляем предыдущие сообщения в чате
        await delete_previous_messages(chat_id, args[0].bot)
