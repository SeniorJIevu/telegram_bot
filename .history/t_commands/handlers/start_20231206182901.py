...
async def start(call: types.CallbackQuery, state: FSMContext):
    ...
    button = types.InlineKeyboardButton(text="Начать", callback_data='Начать')
    keyboard.add(button)
    await call.message.answer("Нажмите кнопку, чтобы начать", reply_markup=keyboard)
    await OrderStart.send_start.set()

@dp.message_handler(lambda call: call.data == "Начать", state=OrderStart.send_start)
