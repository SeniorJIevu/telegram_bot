...
class AntiSpamMiddleware(BaseMiddleware):
    ...
    async def on_pre_process_callback_query(self, callback_query, data):
        ...
        last_click_time = self.last_click_times.get(key, 0)

        # Проверяем, прошло ли достаточно времени с последнего нажатия
        current_time = time.time()
        if current_time - last_click_time < self.cooldown_time:
            # Если прошло меньше cooldown_time секунд, пропускаем запрос
            return False

        # Обновляем время последнего нажатия пользователя на данную кнопку
...
dp.middleware.setup(AntiSpamMiddleware(cooldown_time=5))
MINIO_CLIENT = Minio(
    env["MINIO_SERVER"],
