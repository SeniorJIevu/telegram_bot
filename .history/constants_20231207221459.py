...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def on_process_callback_query(self, query: types.CallbackQuery, data: dict):
        ...
        # Если обработчик был настроен, получаем ограничение и ключ от обработчика
        if handler:
        ...
        # Используем метод Dispatcher.throttle.
        try:
    ...
    async def callback_query_throttled(self, query: types.CallbackQuery, throttled: Throttled):
        ...
        # Рассчитываем, сколько времени осталось до окончания блокировки
        delta = throttled.rate - throttled.delta

        # Предотвращаем флуд
        if throttled.exceeded_count <= 2:
            await query.answer('Слишком много запросов для обратного вызова! ', show_alert=True)
        ...
        # Если текущий обратный вызов не последний с текущим ключом - не отправляем сообщение
        if thr.exceeded_count == throttled.exceeded_count:
            await query.answer('Разблокировано для обратного вызова.')
