...
def callback_rate_limit(limit: int, key=None):
    """
    Decorator for configuring rate limit and key in different callback functions.
...
class CallbackThrottlingMiddleware(BaseMiddleware):
    """
    Simple middleware for callback queries
    """

    def __init__(self, limit=DEFAULT_RATE_LIMIT, key_prefix='callback_antiflood_'):
        self.rate_limit = limit
        self.prefix = key_prefix
        super(CallbackThrottlingMiddleware, self).__init__()
    ...
    async def on_process_callback_query(self, callback_query: types.CallbackQuery, data: dict):
        ...
        """
        # Get current callback handler
        callback_handler = current_callback_handler.get()

        # Get dispatcher from context
        dispatcher = Dispatcher.get_current()

        # If callback handler was configured, get rate limit and key from handler
        if callback_handler:
            limit = getattr(callback_handler, 'throttling_rate_limit', self.rate_limit)
            key = getattr(callback_handler, 'throttling_key', f"{self.prefix}_{callback_handler.__name__}")
        else:
            limit = self.rate_limit
        ...
        try:
            ...
            # Cancel current callback handler
            raise CancelHandler()
    ...
    async def callback_query_throttled(self, callback_query: types.CallbackQuery, throttled: Throttled):
        ...
        # Prevent flooding
        if throttled.exceeded_count <= 2:
            await callback_query.answer('Too many requests! ')
        ...
        # If current callback query is not last with current key - do not send message
        if thr.exceeded_count == throttled.exceeded_count:
            await callback_query.answer('Unlocked.')
