...
class ButtonMiddleware:
    ...
    async def process_message(self, message):
        ...
        if chat_id in self.message_timestamps:
            if current_time - self.message_timestamps[chat_id] < 1:  # Проверяем, прошло ли менее 1 секунды с последнего сообщения
                return True  # Если прошло менее 1 секунды, игнорируем сообщение
            else:
                self.message_timestamps[chat_id] = current_time  # Обновляем временную метку
        else:
            self.message_timestamps[chat_id] = current_time  # Устанавливаем временную метку для нового чата

    async def process_callback(self, callback_query, callback_data, data):
        user_id = callback_query.from_user.id
        if user_id in self.users_in_timeout:
            await callback_query.message.answer("Please press the buttons slower.")
            await asyncio.sleep(3)
        else:
            should_ignore_message = await self.process_message(callback_query.message)
