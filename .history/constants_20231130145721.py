...
def callback_limit(limit: int, key=None):
    """
    Decorator for configuring rate limit and key in different functions.
...
class ThrottlingCallbackMiddleware(BaseMiddleware):
    ...
    async def message_throttled(self, call: types.CallbackQuery, throttled: Throttled):
        ...
        # Prevent flooding
        if throttled.exceeded_count <= 2:
            await call.message.answer('Слишком много идентичных запросов, ждите ! :) ')
        ...
        # If current message is not last with current key - do not send message
        if thr.exceeded_count == throttled.exceeded_count:
            await call.message.answer('Вы снова можете писать :)')
