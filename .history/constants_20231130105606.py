...
# Класс Middleware для блокировки кнопок на определенное время после нажатия
class ButtonMiddleware(BaseMiddleware):
    async def on_pre_process_callback_query(self, query: types.CallbackQuery, data: dict):
        if query.message:
            ...
            # Запуск задачи на разблокировку после 3 секунд
            asyncio.create_task(self.delayed_unlock(state))

    async def delayed_unlock(self, state):
