...
class ButtonMiddleware(BaseMiddleware):
    async def on_pre_process_callback_query(self, query: types.CallbackQuery, data: dict):
        if query.message:
            ...
            # Получение счетчика нажатий
            hits = await state.get_data()

            if hits and hits.get('button_hits', 0) >= button_limit:
                await bot.answer_callback_query(query.id, text="Слишком много нажатий. Подождите немного.")
                return

            # Увеличение счетчика нажатий
            await state.update_data(button_hits=(hits.get('button_hits', 0) + 1))

            # Блокировка кнопок
            await state.set_state('button_clicked')

            # Запуск задачи на сброс счетчика после заданной задержки
            asyncio.create_task(self.delayed_reset(state, delay))

    async def delayed_reset(self, state, delay):
        await asyncio.sleep(delay)
        await state.reset_state()
        await state.update_data(button_hits=0)  # Сброс счетчика нажатий
