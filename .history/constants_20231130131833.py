...
class MessageRateLimitMiddleware(BaseMiddleware):
    ...
    async def on_process_message(self, message: types.Message, data: dict):
        ...
        if (chat_id, message_id) in self.last_message_ids and current_time - self.last_message_ids[(chat_id, message_id)] < 10:
            await message.reply("Please wait for 10 seconds before sending the same message again.")
            return False
        else:
            self.last_message_ids[(chat_id, message_id)] = current_time
            await self.update_last_message_id(chat_id, message_id, current_time)
            return True

    async def update_last_message_id(self, chat_id, message_id, current_time):
        # Add your implementation to update the last message id here
        pass
...
dp.middleware.setup(MessageRateLimitMiddleware())
