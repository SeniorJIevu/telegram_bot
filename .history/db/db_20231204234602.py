...
Пожалуйста пройдите авторизацию, нажав команду /start
        ...
        return
    if pre_checkout_q.invoice_payload == IMAGE_COLLAGE_100:
...
async def successful_payment(message: types.Message, db: asyncpg.pool.Pool):
    ...
    print('a')
    if message.successful_payment.invoice_payload == TARIF_PREMIUM:
...
Очередь — {len(count_in_queue)} человек.
    ...
    if message.successful_payment.invoice_payload == IMAGE_COLLAGE_100:
    ...
    if collage_link:
        if collage_link.get("image_collage_link"):
            if collage_link["image_tarif"] == IMAGE_COLLAGE_100:
