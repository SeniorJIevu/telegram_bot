...
class AntiSpamInlineButtonsMiddleware(BaseMiddleware):
    async def on_pre_process_callback_query(self, callback_query, data):
        user_id = callback_query.from_user.id
        button_id = callback_query.data

        # Создаем уникальный идентификатор для пользователя и кнопки
        key = f"{user_id}_{button_id}"

        # Проверяем, есть ли у пользователя время последнего нажатия на данную кнопку
        if await self.storage.get(key):
            # Если есть запись, значит, запрос был обработан
            return False

        # Записываем факт обработки запроса
        await self.storage.set(key, 1, self.cooldown_time)

        # Пропускаем запрос для обработки
        return True
...
dp.middleware.setup(AntiSpamInlineButtonsMiddleware(cooldown_time=5))
MINIO_CLIENT = Minio(
    env["MINIO_SERVER"],
