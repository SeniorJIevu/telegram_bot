...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def on_process_object(self, obj: TelegramObject, data: dict):
        """
    ...
    async def object_throttled(self, obj: TelegramObject, throttled: Throttled):
        """
        ...
        if throttled.exceeded_count <= 2:
                await obj.reply("Слишком много запросов. Пожалуйста, подождите некоторое время.")
        ...
        if thr.exceeded_count == throttled.exceeded_count:
            await obj.reply("Теперь вы можете продолжить использование бота.")
