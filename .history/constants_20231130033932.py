...
class InlineButtonClickThrottleMiddleware(BaseMiddleware):
    ...
    async def schedule_unmute(self, user_id):
        # Устанавливаем таймер размута пользователя через cooldown_time секунд
        await asyncio.sleep(self.cooldown_time)
        # Обнуляем время последнего нажатия при размуте пользователя
        if user_id in self.last_click_times:
            del self.last_click_times[user_id]
