...
class ButtonRateLimitMiddleware(BaseMiddleware):
    ...
    async def on_process_callback_query(self, query: types.CallbackQuery, data: dict):
        ...
        if button_id in self.last_button_ids and current_time - self.last_button_ids[button_id] < 10:
            if button_id in self.message_count and self.message_count[button_id] >= 1:
                await query.answer("You have reached the message limit for this button. Please wait before pressing it again.")
                return False
            else:
                if button_id in self.message_count:
                    self.message_count[button_id] += 1
                else:
                    self.message_count[button_id] = 1
                await self.send_message(query, data)  # Отправить сообщение
                return True
        else:
    ...
    async def send_message(self, query: types.CallbackQuery, data: dict):
