...
async def start(message: types.Message, state: FSMContext,  db: asyncpg.pool.Pool):
    is_user = await check_user_db(message, db)
    if not is_user:  
        print('Работает')
        ...
        text="""
                ❗️Данная нейросеть предназначена для использования лицами старше 18 лет ❗️
🔞 Чтобы продолжить подтвердите свой возраст 🔞
        ...
        await send_start_message(message, state, db)       
...
async def handle_start(message: types.Message, state: FSMContext, db: asyncpg.pool.Pool):
    await message.answer(cache_time=20)
    text = """
...
@dp.callback_query_handler(state=OrderStart.send_start)
async def send_start(
    ...
    text = """Привет 😈
Воплоти свои фантазии с помощью нейросети 🔞
Этот бот может сгенерировать множество сочных картинок по обычным фотографиям ❤️🔞😈

P.s. Мы не храним ваши персональные данные, а платежи проходят анонимно 💪✅
    """
    try:
        ...
        if is_user_exist and await get_user_image_count(call, db):
            text = "<i><b>Привет 😈</b></i>"
...
P.s. Мы не храним ваши персональные данные, а платежи проходят анонимно 💪✅
    ...
    try:
        ...
        await state.set_state(OrderStart.send_start.state)
        if is_user_exist and await get_user_image_count(message, db):
            text = "<i><b>Привет 😈</b></i>"
...
async def send_continue(
    ...
    print('send_continue')
    try:
        text = "Для продолжения выберите пол, нажав на одну из кнопок ниже 👇"
...
👉 Поставить галочку
☑️ Сжать изображение
👉 Четкое, в хорошем качестве, селфи
👉 Максимум 1 человек на фото
...
👉 Животные
👉 Часть лица прикрыта
👉 Плохое качество, размытость
👉 Видео, Tg кружки
👉 Фото без сжатия

А теперь отправь одну фотографию боту 👇
    """
