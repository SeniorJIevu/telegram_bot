...
async def one_users_queues(call, db: asyncpg.pool.Pool):
    async with db.acquire() as connection:
                    user_queue = await connection.fetch(
                        "SELECT * FROM queue WHERE user_id = $1",
                        call.message.chat.id,
                    )
                    print('user_queue', user_queue, type(user_queue),len(user_queue) == 0, len(user_queue) != 0)
                    if len(user_queue) != 0:
                        return False
                    else:
                        return True
...
async def check_user_photo(message, db: asyncpg.pool.Pool):
        if type(message) == types.CallbackQuery:
            async with db.acquire() as connection:
                photos = await connection.fetch(
                    "SELECT image_reference_link FROM history_image_links WHERE user_id = $1",
                    message.message.chat.id,
                )
                if photos:
                    
                    photos_lists =[j.replace(';;', '').split('jpg')  for i in photos for j in i]
                    result_ref_from_user = {j + 'jpg':'' for i in photos_lists for j in i}
                   
                    

                    
                    return result_ref_from_user
                else:
                    photo = list()
                    return photo
        else:
...
async def check_user_db(message, db: asyncpg.pool.Pool):
...
async def check_free_queue(request_type, db: asyncpg.pool.Pool):
    ...
    async with db.acquire() as connection:
        query_select_id = """
        SELECT user_id 
        FROM queue WHERE request_type=$1 OR request_type=$2 OR request_type=$3 
        """
...
async def remove_histoty_reference(message, db: asyncpg.pool.Pool):
    async with db.acquire() as connection:
        async with connection.transaction():
            query_delete = """
                DELETE FROM history_image_links
                WHERE user_id = $1
            """
            values_delete = (
                message.chat.id,
